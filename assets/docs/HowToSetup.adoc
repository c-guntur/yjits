...
* *JDK 22* installed on your computer
...
** The toolchains point to JDK 8 and 22 used in the project.
...
. Verify that the module *java22* uses *JDK 22* (experimental features should be allowed).
. Run the App.java under link:../../java22/src/main/java/conf/App.java[App.java]
. Verify output looks structurally similar to the one from java8.

=== Update the Java 8 module to use Java 22

. Find instances of FIXME #0 in the java8 link:../../java8/pom.xml[pom.xml]. Fix per advise.
. Ensure that the module java8 now uses Java 22 as its JDK.
. Follow the numbered FIXME instructions to learn great features added to Java from Java 8 until Java 22!
